...
class Proxy:
	def __init__(self,countries:list=[],protocol:str='http',maxProxies:int = 10,autoRotate:bool = False,cachePeriod:int = 10):
		'''
	...
	def updateProxyList(self):
		try:
			...
			if cacheState:
				log('info',f'{datetime.now(timezone.utc).time()} Loaded proxies from cache')
				self.proxies = data[1]
				self.current = self.proxies[0]
				return
			else:
				log('info',f'{datetime.now(timezone.utc).time()} Cache expired. Updating cache...')
		...
		if len(self.proxies) == 0:
			log('warn','No proxies found for current settings. To prevent runtime error updating the proxy list again.')
			self.update()
