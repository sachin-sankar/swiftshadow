	...
	def update(self):
		...
		try:
			with open('.swiftshadow.dat','rb') as file:
				data = load(file)
				cacheState = self.checkCache(raw.split('UTC')[0].split('Updated at')[-1].strip(),data[0])
			if cacheState:
		...
		raw = [ i.split('<td>') for i in raw.split('<tbody>')[1].split('</tbody>')[0].split('</tr><tr>')]
		raw = [[i[1].rstrip('</td>'),i[2].rstrip('</td>'),i[3].split('<',1)[0],i[-1].split('hx',1)[1][2]] for i in raw]
		final = [ [i[0]+':'+i[1],i[2],'https' if i[3]=='y' else 'http'] for i in raw]
		self.proxies = []
		for i in final:
		...
		if len(self.proxies) == 0:
			print('[ swiftshadow ] No proxies found for current settings. To prevent runtime error updating the proxy list again.')
			self.update()
